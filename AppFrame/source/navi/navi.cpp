...
bool Navi::FindPath(const vec::Vec3& startpos, const vec::Vec3& goalpos, at::vec<vec::Vec3>& path, float radius)
{
	...
	for(auto&& navmesh : _nNavigationMesh)
	{
		...
		if(DxlibConverter::HitCheckLineToTriangle(
			startpos + vec::Vec3(0.0f, 1.0f, 0.0f) * navi::VERTICAL_DIS,	// 上から
			startpos + vec::Vec3(0.0f, -1.0f, 0.0f) * navi::VERTICAL_DIS,	// 下まで
			navmesh.GetMesh()._vVertex1,									// 三角形の頂点１
			navmesh.GetMesh()._vVertex2,									// 三角形の頂点２
			navmesh.GetMesh()._vVertex3									// 三角形の頂点３
		).HitFlag == 1)
		{
			start = &navmesh;		// 見つけた
		}
		...
		// 終了地点
		if(DxlibConverter::HitCheckLineToTriangle(
			goalpos + vec::Vec3(0.0f, 1.0f, 0.0f) * navi::VERTICAL_DIS,	// 上から
			goalpos + vec::Vec3(0.0f, -1.0f, 0.0f) * navi::VERTICAL_DIS,	// 下まで
			navmesh.GetMesh()._vVertex1,									// 三角形の頂点１
			navmesh.GetMesh()._vVertex2,									// 三角形の頂点２
			navmesh.GetMesh()._vVertex3									// 三角形の頂点３
		).HitFlag == 1)
...
void Navi::GetPolygonData(int handle, at::vec<Polygon3D>& polygon)
{
	...
	for(int i = 0; i < list.PolygonNum; i++)
	{
		Polygon3D myPolygon
		(
			DxlibConverter::DxLibToVec(dxVertex[dxPolygon[i].VIndex[0]].Position),		// 頂点１
			DxlibConverter::DxLibToVec(dxVertex[dxPolygon[i].VIndex[1]].Position),		// 頂点２
			DxlibConverter::DxLibToVec(dxVertex[dxPolygon[i].VIndex[2]].Position)		// 頂点３
		);
